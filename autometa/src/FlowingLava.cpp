...
void FlowingLava::Tick()
{
	...
	for (size_t x = 0; x < SIZE; x++)
	{
		for (size_t y = 0; y < SIZE; y++)
		{
			...
			m_soupHeatMap[x][y] = localSoupHeatLevel / 10 + localPotHeatLevel / 4 * 0.8; // /9.5 instead of /10 for better / brighter lava
...
std::array<std::array<std::array<unsigned char, 4>, SIZE>, SIZE> FlowingLava::GetData()
{
	m_yOffset %= SIZE *4;
	for (size_t x = 0; x < SIZE; x++)
	{
		for (size_t y = 0; y < SIZE; y++)
		{
			float colorHeat = std::max<float>(std::min<float>(2.0f * m_soupHeatMap[(x + m_yOffset/4) % SIZE][y], 1.0), 0.0);
