	    ...
	    for (size_t x = 0; x < SIZE; x++)
	{
		for (size_t y = 0; y < SIZE; y++)
		{
			...
			float localSoupHeatLevel = 
				...
				m_soupHeatMap[(x-1 + offsets[0]) % SIZE][(y+2 + offsets[1]) % SIZE] +//-11
				m_soupHeatMap[(x    + offsets[0]) % SIZE][(y+2 + offsets[1]) % SIZE] +//01
				m_soupHeatMap[(x+1 + offsets[0]) % SIZE][(y+2+ offsets[1]) % SIZE]+//11
				
				m_soupHeatMap[(x-1 + offsets[0]) % SIZE][(y    + offsets[1]) % SIZE] +//-1-1
			...
			float localPotHeatLevel = 
				...
				m_potMap[(x - 1)     % SIZE][(y - 3) % SIZE];//11 
			//std::cout << localSoupHeatLevel << "\n";


			m_soupHeatMap[x][y] = localSoupHeatLevel / 10 + localPotHeatLevel / 5; // /9.5 instead of /10 for better / brighter lava //6.66666667 =(6+6/9)

			m_potMap[x][y] += m_flameMap[x][y]*0.01f;
			if (m_potMap[x][y] < 0.0f) m_potMap[x][y] = 0.0f;

			m_flameMap[x][y] -= 0.06f;
			if ( (float)rand()/RAND_MAX <= 0.005f) m_flameMap[x][y] = 1.25f;
...
std::array<std::array<std::array<unsigned char, 4>, SIZE>, SIZE> UnLava::GetData()
{
	    ...
	    for (size_t x = 0; x < SIZE; x++)
	{
		for (size_t y = 0; y < SIZE; y++)
		{
			float colorHeat = std::min<float>(   1.5f * m_soupHeatMap[x][y],  1.0);
